...
function longestCommonPrefix(strs: string[]): string {
  ...
  while (maxLength <= max.length) {
    ...
    if (checkIfSubExistsInAll(lengthList, sub)) return sub;
    else maxLength++;
  }
...
const checkIfSubExistsInAll = (
  lengthList: { length: number; str: string }[],
  sub: string
): boolean => {
  let index = 0;
  for (let i = 0; i < lengthList.length; i++) {
    if (!lengthList[i].str.startsWith(sub)) break;
    index = i;
  }
